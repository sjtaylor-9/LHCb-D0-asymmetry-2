...
#include "include/Pythia8/Pythia.h" // Include Pythia headers.
#include <TFile.h>
#include <TNtuple.h>
#include <iostream>
#include <fstream>
...
int main(int argc, char* argv[]) {
  ...
  pythia.readString("PhaseSpace:pTHatMin = 0.4"); // 400 MeV

  // Beam parameters, assuming no crossing angle
  pythia.readString("Beams:eCM = 7000."); // 8 TeV CM energy; default = 14                Suzanne had this as 7000 is it 14000?
  ...
  outputfile.open((std::string("/afs/cern.ch/user/s/sjtaylor/WorkSpace/D0_production_asymmetry_Sem2/LHCb_D0_asymmetry_2/pythia_hadronisation/pythia_7TeV_")+std::string(argv[1])+std::string(".txt")));
  ...
  for (int iEvent = 0; iEvent < nEvents; ++iEvent){
    ...
    for (int i = 0; i < pythia.event.size(); ++i)
        ...
        if (abs( pythia.event[i].id()) == 431){
            iDs = i;
            if (abs( pythia.event[iDs+1].id() ) != 431) {
